...
form.addEventListener('submit', (event) => {
    ...
    // If returned value is an empty number || goes out of range
    if (isNaN(value) || value <= 0 || value > 9) {
        result.innerHTML = `<p class="result">${text[random]}</p>`
    } else {

        // Slice up to N Index and Return a copy of the array
        let tempText = text.slice(0, value)

        // Dress each item on a paragraph and join into string
        tempText = tempText.map((item) => {
            return `<p class="result">${item}</p>`
        }).join("")


        // Inject HTML into Parent Container
